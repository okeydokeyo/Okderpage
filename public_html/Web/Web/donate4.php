...
<body>
    ...
    //********************************************************************************************************
    //檔案下載標籤管理查詢
    $sql = "select * from `download` where `DB_DowTagID`=14 $DB_DowUnitID ORDER BY `DB_DowSort` ASC";	
    $return = iron_page( $sql, 10, 10, $page, 10 ); //iron分頁程式
    $result = mysql_query($sql) or die("查詢失敗");
    $number = mysql_num_rows($result); //全部資料的總數
    $url = "donate_4.php"; //本頁的網址 & 使用的 get變數
    
    ?>
    <form action="donate4.php" method="GET" name="form1" style="margin-top:12em; margin-left:6em;" > 
		  <div align="left"> 選擇檔案類別：         
	           <select id="DB_DowUnitID" name="DB_DowUnitID" class="text_12px_01b">
		          <option value="" >不分類</option>
		          <?//查詢檔案下載類別資料
		          $dowun_result = mysql_query("select * from `download_unit` where `DB_DowTagID`=14 ORDER BY `DB_DowUnitSort` ASC") or die("查詢失敗");
	           ...
	           <input name="no" type="hidden" value="14" />
		...
		<table width="50%" border="0" cellspacing="0" cellpadding="0" id="margin_01" class="text_12px_01" summary="捐款捐物徵信表格" align="center">
		  <caption align="left" class="hidden">年度收支表</caption>
		  <tr>

...
$CalStartTime = $_POST['CalStartTime'];
$CalEndTime = $_POST['CalEndTime'];
...
if ( !empty($DB_CalEndTime) && !empty($DB_CalStartTime) && !empty($DB_CalTitle) && !empty($DB_CalContent) ){			
				  ...
				  $message = iconv("UTF-8", "Big5","======================&lt;內政部建築研究所 風雨風洞實驗室-行事曆&gt;====================<br>
					...
					$mail->From     = "s9863570@m98.nthu.edu.tw";
					...
					//行事曆標籤管理查詢
                    $ary3 = SoloSql("calendar_tags","`DB_CalTagID`='$CalTagID'");
					$day = date("z",strtotime($DB_CalEndTime)-strtotime($DB_CalStartTime)); //轉成天數
						
					if ($DB_CalStartTime == $CalStartTime && $DB_CalEndTime == $CalEndTime){  //辨別原始時間是否相同
					        if ($DB_CalCross == $CalCross){  //辨別跨六日情形是否相同
					                    $UpStr = "`DB_CalCompany`='$DB_CalCompany',`DB_CalFacilities`='$DB_CalFacilities',`DB_CalTitle`='$DB_CalTitle',`DB_CalContent`='$DB_CalContent',`DB_CalAnnounce`='$DB_CalAnnounce',`DB_EndTime`=NOW(),`DB_EditUser`='".$_SESSION['ManUser']."'";
                                        mysql_query("update `calendar` set ".$UpStr." where `DB_CalGroup`='$DB_CalGroup'") or die("修改失敗1!!");
						    }else{
										//刪除原始資料
										mysql_query("delete from `calendar` where `DB_CalGroup`='$DB_CalGroup'") or die("刪除失敗1!!");
										...
										//刪除原始資料
										mysql_query("delete from `calendar` where `DB_CalGroup`='$DB_CalGroup'") or die("刪除失敗2!!");
										
										//重建資料
										for ($a2=0 ;$a2<=$day ;$a2++){	
											$ckW = date("w",strtotime($DB_CalStartTime)+($a2*86400));        //轉成星期
											$ckDate = date("Y-m-d",strtotime($DB_CalStartTime)+($a2*86400)); //轉成日期
											...
											if ($DB_CalCross == "0" && $ckW == "6" || $DB_CalCross == "0" && $ckW == "0"){
											   ...
											   $UpStr2 = "'$CalTagID','$DB_CalGroup','$ckDate','$DB_CalCross','$DB_CalCompany','$DB_CalFacilities','$DB_CalTitle','$DB_CalContent','$DB_CalAnnounce',NOW(),'".$_SESSION['ManUser']."'";
												...
												mysql_query("insert into `calendar` (".$UpStr.") values (".$UpStr2.")") or die("新增失敗2!!");
											}
										}					
					}	
					...
					$UpStr2="'".$_SESSION['ManUser']."','".$_SERVER['REMOTE_ADDR']."','行事曆管理-".ereg_replace("'","\'",$ary3['DB_CalTagSubject'])."','".$DB_CalTitle."','edit',NOW()";
					Recording_Add("recording",$UpStr,$UpStr2);
					
					parent_url_msg("修改成功!!","calendar_list.php?DB_CalTagID=$CalTagID");
...
<!--menu_end-->
	...
	<table border="0" cellspacing="0" cellpadding="0">
	  <tr>
		...
		<td align="left" valign="middle" background="images/gray_02.gif" class="text_12px_01"><a href="id_info.php" class="link_01">首頁</a> >> 網頁選單管理 >> 行事曆管理 >> <? echo $ary2['DB_CalTagSubject'];?> >> <span class="text_12px_02"><strong>修改行事曆</strong></span></td>
...
<form action="calendar_edit.php" method="POST" name="form1" target="FormFrame">	 
	  <tr>
		...
		<table width="100%" border="0" cellspacing="1" cellpadding="5" class="text_12px_01">
		  ...
		  <tr>
			...
			<td width="85%" align="left" valign="top" class="border_02">
			  ...
			  ?>			  
			  <input id="StartTime" name="DB_CalStartTime" value="<? echo $calendar2_ary['DB_CalStartTime'];?>" type="text" class="text_12px_01"  readonly=""/> 
			  ...
			  ～			  
			  <input id="EndTime" name="DB_CalEndTime" value="<? echo $calendar_ary['DB_CalStartTime'];?>" type="text" class="text_12px_01" readonly=""/> 
			  ...
			  <input name="DB_CalAnnounce" type="radio" value="0" <? if ($ary['DB_CalAnnounce'] == "0"){echo "checked";}?>/>顯示
			  <input name="DB_CalAnnounce" type="radio" value="1" <? if ($ary['DB_CalAnnounce'] == "1"){echo "checked";}?>/>不顯示
			  <input type="hidden" name="DB_CalID" value="<? echo $ary['DB_CalID'];?>">
			  <input type="hidden" name="CalTagID" value="<? echo $_GET['DB_CalTagID'];?>">
			  <input type="hidden" name="DB_CalGroup" value="<? echo $ary['DB_CalGroup'];?>">
			  <input type="hidden" name="CalStartTime" value="<? echo $ary['DB_CalStartTime'];?>">
			  <input type="hidden" name="CalEndTime" value="<? echo $calendar_ary['DB_CalStartTime'];?>">
			  <input type="hidden" name="CalCross" value="<? echo $ary['DB_CalCross'];?>">
		    </td>
		  </tr>
		  <tr>
			...
			<td align="left" valign="top" class="border_02"><input name="DB_CalCompany" value="<? echo $ary['DB_CalCompany'];?>" type="text" class="text_12px_01" size="80" /></td>
		  </tr>
		  <tr>
			...
			<td align="left" valign="top" class="border_02"><input name="DB_CalFacilities" value="<? echo $ary['DB_CalFacilities'];?>" type="text" class="text_12px_01" size="80" /></td>
		  </tr>
		  <tr>
			...
			<td align=" left" valign="top" class="border_02">
			     <input name="DB_CalCross" type="radio" value="0" <? if ($ary['DB_CalCross'] == "0"){echo "checked";}?>/>是
			     <input name="DB_CalCross" type="radio" value="1" <? if ($ary['DB_CalCross'] == "1"){echo "checked";}?>/>否
			</td>
		  </tr>
		  <tr>
			...
			<td align="left" valign="top" class="border_02"><input name="DB_CalTitle" value="<? echo $ary['DB_CalTitle'];?>" type="text" class="text_12px_01" size="80" /></td>
		  </tr>
		  <tr>
			...
			<td width="85%" align="left" valign="top" class="border_02"><textarea name="DB_CalContent" cols="110" rows="5" class="text_12px_01"><? echo $ary['DB_CalContent'];?></textarea></td>
		  </tr>
		</table>		
		<div align="center" style="padding:5px;margin:5px"><a href="javascript:document.form1.submit();" onClick="return checkinput();" class="button_01">修改資料</a></div>
		</td>

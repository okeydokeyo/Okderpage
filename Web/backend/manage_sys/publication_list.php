...
$DB_LefID=$_GET['DB_LefID']; //友善新世界=100
$DB_LefNumID = $_GET['DB_LefNumID'];
$arry1=SoloSql("left"," `DB_LefID`='$DB_LefID'");
$page = (empty($_GET['page']))?1:$_GET['page']; //現在頁面

//條列式訊息管理查詢
$sql = "select * from `ordi` where `DB_OrdKind`='$DB_LefNumID' ORDER BY `DB_OrdTime` DESC";	
$return = iron_page( $sql, 10, 10, $page, 10 ); //iron分頁程式
$result = mysql_query($sql) or die("查詢失敗");
$number = mysql_num_rows($result); //全部資料的總數
$url = "publication_list.php"; //本頁的網址 & 使用的 get變數
    ...
    <tr>
	  ...
	  <tr>
		<td height="10" colspan="3" align="left" valign="top"><div align="right" style="padding:5px;margin:5px">
		  <a href="publication_calss.php" class="button_04">◎回標籤列表</a>&nbsp;
            <a href="publication_add.php?DB_LefNumID=<? echo $DB_LefNumID;?>" class="button_04">◎新增資料</a></div></td>
	  </tr>
            </table>
	  ...
	  <tr>
		...
		<? }?>
            ...
            <div align="left" style="padding:5px;margin:5px">
                ...
                <span class="text_12px_04">每頁10筆，共<? echo $number;?>筆資料</span>
                <?php  
                ...
                ?>		  
                <? if ($i!=$page){?>　
                <a href="javascript:GoPage(<? echo $i;?>)" class="link_02"><? }?><? if ($i==$page){?>　<span class="text_12px_03b"><? }?><? echo $i;?><? if ($i==$page){?></span><? }?><? if ($i!=$page){?></a>
                <? }?> 
                <?}?>　		  

...
<span id="OpenDIV_4" style="display:<? if ($ary['DB_LefTagLayer'] == "1" && $ary['DB_LefTagClass'] == "1"){echo "block";}else{echo "none";}?>">			
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="text_12px_01">
			  ...
			  <option value="3" <? if ($ary['DB_LefTagBasis'] == "3"){echo "selected";}?>>行事曆</option>
			  ...
			  <option value="7" <? if ($ary['DB_LefTagBasis'] == "7"){echo "selected";}?>>網站連結</option>
			  <option value="8" <? if ($ary['DB_LefTagBasis'] == "8"){echo "selected";}?>>參訪紀錄</option>
			  </select>
		<span id="show1">
		 <? if (/*$ary['DB_LefTagBasis'] != "2" && $ary['DB_LefTagBasis'] != "3" &&*/ $ary['DB_LefTagBasis'] != "7" && $ary['DB_LefTagBasis'] != "8"){?>
			  <select name="DB_LefTagNumID" class="text_12px_01">
			  <?
			   if ($ary['DB_LefTagBasis'] == "1"){
			        $BaSql = "ordi_tags";
					$BaSort = "DB_OrdTagSort";
					$BaID = "DB_OrdTagID";
					$BaName = "DB_OrdTagSubject";
			   }else if ($ary['DB_LefTagBasis'] == "2"){
			        $BaSql = "article";
					$BaSort = "DB_ArtSort";
					$BaID = "DB_ArtID";
					$BaName = "DB_ArtSubject";					
			   }else if ($ary['DB_LefTagBasis'] == "3"){
			        $BaSql = "calendar_tags";
					$BaSort = "DB_CalTagSort";
					$BaID = "DB_CalTagID";
					$BaName = "DB_CalTagSubject";					
			   }else if ($ary['DB_LefTagBasis'] == "4"){
			        $BaSql = "download_tags";
					$BaSort = "DB_DowTagSort";
					$BaID = "DB_DowTagID";
					$BaName = "DB_DowTagSubject";					
			   }else if ($ary['DB_LefTagBasis'] == "5"){
			        $BaSql = "life_tags";
					$BaSort = "DB_LifTagSort";
					$BaID = "DB_LifTagID";
					$BaName = "DB_LifTagSubject";					
			   }else if ($ary['DB_LefTagBasis'] == "6"){
			        $BaSql = "faq_tags";
			  ...
			  ?>
			    <option value="<? echo $BaSq_ary[''.$BaID.''];?>" <? if ($ary['DB_LefTagNumID'] == $BaSq_ary[''.$BaID.'']){echo "selected";}?>><? echo $BaSq_ary[''.$BaName.''];?></option>
				<? } ?>
			  </select>
		 <? } ?>	
	    </span>		
...
<span id="OpenDIV_3" style="display:<? if ($ary['DB_LefTagLayer'] == "1" && $ary['DB_LefTagClass'] == "3"){echo "block";}else{echo "none";}?>">			  
		  ...
		  <tr>
			...
			<td align="left" valign="top" class="border_02">
			  ...
			  <input name="DB_LefTagAnnounce" type="radio" value="1" <? if ($ary['DB_LefTagAnnounce'] == "1"){echo "checked";}?>/>否</td>
		  </tr>
		</table>
